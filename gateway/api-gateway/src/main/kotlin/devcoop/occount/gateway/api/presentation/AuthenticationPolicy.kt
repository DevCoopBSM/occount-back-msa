...
class AuthenticationPolicy {
    private val rules = authenticationRules {
        ...
        rule(HttpMethod.GET, "/api/v3/contributions/deposit-requests").adminOnly()
        rule(HttpMethod.PATCH, "/api/v3/contributions/deposit-requests/{requestId}/approve").adminOnly()
        rule(HttpMethod.PATCH, "/api/v3/contributions/deposit-requests/{requestId}/reject").adminOnly()
        rule(HttpMethod.GET, "/api/v3/contributions/withdrawal-requests").adminOnly()
        rule(HttpMethod.PATCH, "/api/v3/contributions/withdrawal-requests/{requestId}/approve").adminOnly()
        rule(HttpMethod.PATCH, "/api/v3/contributions/withdrawal-requests/{requestId}/reject").adminOnly()
        rule("/api/v3/users/**").authenticated()
        rule("/api/v3/contributions/**").authenticated()
