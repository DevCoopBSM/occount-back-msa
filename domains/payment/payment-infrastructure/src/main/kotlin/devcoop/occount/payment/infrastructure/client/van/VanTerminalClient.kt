...
class VanTerminalClient(
    ...
    private fun doExecuteTransaction(actionName: String, requestMessage: ByteArray): VanResult {
        if (!socketConnection.ensureConnected()) {
    ...
    private fun waitForResponse(actionName: String): VanResult {
        ...
        while (System.nanoTime() < deadline) {
            ...
            socketConnection.logMessage("수신", response)
            val responseHex = protocolSpec.toHex(response)

            when {
                response.firstOrNull() == protocolSpec.stxByte -> {
                    val parsed = messageParser.parsePaymentResponse(response)
                    sendAck()
                ...
                responseHex == protocolSpec.dleHex -> {
                    if (lastStxResponse == null) {
                        return userCancelledResult()
                    }
